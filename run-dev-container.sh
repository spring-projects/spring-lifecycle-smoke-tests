...
docker run -it --entrypoint /bin/bash --privileged -v $HOST_WORK_DIR:$CONTAINER_WORK_DIR:delegated -w $CONTAINER_WORK_DIR spring-checkpoint-restore-smoke-tests-dev -c 'source /docker-lib.sh && start_docker && bash'