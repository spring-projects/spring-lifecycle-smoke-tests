...
@ApplicationTest
public class DataCassandraApplicationTest {

	@Test
	void connectionTest(AssertableOutput output) {
		Awaitility.await().atMost(Duration.ofSeconds(10)).untilAsserted(() -> {
			assertThat(output).hasLineMatching("Starting CassandraReader: was (INITIALIZED|STOPPED)");
			assertThat(output).hasLineMatching("count \\d+: User\\{id=\\d, username='.*");
		});
	}
